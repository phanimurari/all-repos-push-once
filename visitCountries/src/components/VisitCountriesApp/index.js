...
import CountryButton from '../CountryButtonItem'
import VisitedCountry from '../VisitedCountryItem'
import {
  BgContainer,
  VisitCountriesBgContainer,
  Heading,
...
class VisitCountriesApp extends Component {
  state = {
    countiesList: initialCountriesList,
  }

  onClickVisit = id => {
    const {countiesList} = this.state
    const updatedList = countiesList.map(each => {
      if (each.id === id) {
        const updatedData = {
          ...each,
          isVisited: true,
  ...
  render() {
    ...
    const visitedCountries = countiesList.filter(
      ...
      <BgContainer>
        <VisitCountriesBgContainer>
          <div>
            ...
            <div>
              <CountryButtonsContainer>
                {countiesList.map(each => (
                  <CountryButton
                    key={each.id}
                    countryButtonData={each}
                  ...
                  <VisitedCountriesContainer>
                    {visitedCountries.map(each => (
                      <VisitedCountry
                        countryData={each}
                        key={each.id}
                        onClickRemoveFromVisit={this.onClickRemoveFromVisit}
                      />
                    ))}
                  </VisitedCountriesContainer>
                )}
